...
class _LaunchPageState extends ConsumerState<LaunchPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref
          ...
          .initialize(widget.modelUrl, widget.modelName);
    });
  }

  void _onDownloadProgress(Map<String, dynamic> event) {
  ...
  void _onDownloadComplete(Map<String, dynamic> event) {
    final category = event['category'] as String?;
    if (category == 'base_config') {
      ref
          .read(launchStateProvider.notifier)
          .updateStatus(LaunchStatus.initial, progress: 0.3);
    } else if (category == 'model') {
      ref
          .read(launchStateProvider.notifier)
          .updateStatus(LaunchStatus.initial, progress: 0.7);
    }
  }

  void _onDownloadFail(Map<String, dynamic> event) {
    final error = event['error'] as String? ?? '下載失敗';
    ref
        .read(launchStateProvider.notifier)
        .updateStatus(LaunchStatus.error, errorMessage: error);
  }
  ...
  void _onInitReady() {
    ...
    ref
        .read(launchStateProvider.notifier)
        .updateStatus(LaunchStatus.completed, progress: 1.0);
    _skipToHome();
  }

  void _onInitError(Map<String, dynamic> event) {
    final error = event['error'] as String? ?? '初始化失敗';
    print('模型初始化錯誤: $error');
    ref
        .read(launchStateProvider.notifier)
        .updateStatus(LaunchStatus.error, errorMessage: error, progress: 0.9);
  }
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              // 進度容器
              _buildProgressCard(launchState),

              // 錯誤狀態
              if (launchState.status == LaunchStatus.error)
                buildLaunchErrorCard(launchState),
            ],
          ),
        ),
      ),
  ...
  Container _buildLoadingIcon() {
    return Container(
      child: Assets.images.icon.loadingIcon.image(
        fit: BoxFit.contain,
        height: 200,
      ),
    );
  ...
  Container _buildProgressCard(LaunchStateData launchState) {
    return Container(
  ...
  Container buildLaunchErrorCard(LaunchStateData launchState) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.red.withAlpha(1),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: Colors.red.withAlpha(1), width: 2),
      ),
      child: Column(
        children: [
